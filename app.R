  ...
  bootstrapPage('',
                ...
                navbarPage(#div(id="title", img(src="meta.png")), #logo
                  ...
                  navbarMenu(div(id="data_div", img(src="data-512.png")),
                             tabPanel("Upload csv",
                                      source("openmeta/ui/ui_data_csv.R", local = TRUE))
                  ),#navbarMenu "Data"
                  ...
                  navbarMenu(div(id="meta_analysis_div", img(src="meta_analysis.png")),
                             tabPanel("Normal approximation",
                                      source("openmeta/ui/ui_meta_norm.R", local = TRUE)),
                             tabPanel("Exact likelihood",
                                      source("openmeta/ui/ui_meta_exact.R", local = TRUE))
                  ...
                  navbarMenu(div(id="cum_meta_analysis_div", img(src="cum_meta_analysis.png")),
                             tabPanel("Accumulative data",
                                      source("openmeta/ui/ui_cum_data.R", local = TRUE)),
                             tabPanel("Normal approximation",
                                      source("openmeta/ui/ui_cum_norm.R", local = TRUE)),
                             tabPanel("Exact likelihood",
                                      source("openmeta/ui/ui_cum_exact.R", local = TRUE))
                  ...
                  tabPanel(div(id="report_div", img(src="report.png")),
                           source("openmeta/ui/ui_report.R", local = TRUE)
                  ),#tabPanel "Report"
...
server <- function(input, output, session) {
  ...
  source("openmeta/support/helpers.R", local = TRUE)
  
  source("openmeta/server/server_data_csv.R", local = TRUE)
  
  source("openmeta/server/server_meta_norm.R", local = TRUE)
  
  source("openmeta/server/server_meta_exact.R", local = TRUE)
  
  source("openmeta/server/server_cum_data.R", local = TRUE)
  
  source("openmeta/server/server_cum_norm.R", local = TRUE)
  
  source("openmeta/server/server_report.R", local = TRUE)
